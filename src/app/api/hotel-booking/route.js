...
export const POST = async (req) => {
  try {
    ...
    if (!formData.contact.email || !formData.contact.phone || 
        !formData.hotel.location || !formData.hotel.checkInDate || !formData.hotel.checkOutDate) {
      return new Response(
        JSON.stringify({ 
    ...
    // Email options
    const mailOptions = {
      from: `"Hotel Booking Form" <${process.env.NEXT_PUBLIC_EMAIL_USER}>`,
      to: process.env.NEXT_PUBLIC_EMAIL_RECEIVER,
      subject: `New Hotel Booking Request - ${formData.hotel.location}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden;">
          ...
          <div style="background: #f1f5f9; padding: 15px; text-align: center; font-size: 12px; color: #64748b;">
            <p style="margin: 0;">This email was sent from the hotel booking form.</p>
          </div>
        </div>
      `,
    };
