...
export const POST = async (req) => {
  ...
  const hotelsHTML = formData.hotels.map((hotel, index) => {
    ...
    return `
      <div style="margin-bottom: 20px; padding: 15px; border: 1px solid #e2e8f0; border-radius: 5px;">
        ...
        <p><strong>Check-In:</strong> ${hotel.checkInDate ? (hotel.checkInDate.includes('T') ? new Date(hotel.checkInDate.split('T')[0] + 'T12:00:00').toISOString().split('T')[0] : new Date(hotel.checkInDate + 'T00:00:00').toISOString().split('T')[0]) : 'Not specified'}</p>
        <p><strong>Check-Out:</strong> ${hotel.checkOutDate ? (hotel.checkOutDate.includes('T') ? new Date(hotel.checkOutDate.split('T')[0] + 'T12:00:00').toISOString().split('T')[0] : new Date(hotel.checkOutDate + 'T00:00:00').toISOString().split('T')[0]) : 'Not specified'}</p>
  ...
  const mailOptions = {
      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden;">
        ...
        <div style="padding: 20px; background: #ffffff;">
          ...
          <p><strong>Visa Interview Date:</strong> ${formData.additional.visaInterviewDate ? 
            (formData.additional.visaInterviewDate.includes('T') ? new Date(formData.additional.visaInterviewDate.split('T')[0] + 'T12:00:00').toISOString().split('T')[0] : new Date(formData.additional.visaInterviewDate + 'T00:00:00').toISOString().split('T')[0]) : 'Not specified'}</p>
          <p><strong>Delivery Date:</strong> ${formData.additional.deliveryDate ? 
            (formData.additional.deliveryDate.includes('T') ? new Date(formData.additional.deliveryDate.split('T')[0] + 'T12:00:00').toISOString().split('T')[0] : new Date(formData.additional.deliveryDate + 'T00:00:00').toISOString().split('T')[0]) : 'Not specified'}</p>
    ...
    const checkInDates = formData.hotels.map(hotel => 
      hotel.checkInDate ? (hotel.checkInDate.includes('T') ? new Date(hotel.checkInDate.split('T')[0] + 'T12:00:00').toISOString().split('T')[0] : new Date(hotel.checkInDate + 'T00:00:00').toISOString().split('T')[0]) : 'Not specified'
    ).join(', ');
    
    const checkOutDates = formData.hotels.map(hotel => 
      hotel.checkOutDate ? (hotel.checkOutDate.includes('T') ? new Date(hotel.checkOutDate.split('T')[0] + 'T12:00:00').toISOString().split('T')[0] : new Date(hotel.checkOutDate + 'T00:00:00').toISOString().split('T')[0]) : 'Not specified'
    ).join(', ');
