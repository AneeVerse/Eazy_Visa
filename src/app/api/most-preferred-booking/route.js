  ...
  try {
    ...
    const flightLegsHTML = formData.flight.legs.map((leg, index) => `
      <div style="margin-bottom: 15px; padding: 10px; border: 1px solid #e2e8f0; border-radius: 5px;">
        ...
        <p><strong>Date:</strong> ${leg.date ? (leg.date.includes('T') ? new Date(leg.date.split('T')[0] + 'T12:00:00').toISOString().split('T')[0] : new Date(leg.date + 'T00:00:00').toISOString().split('T')[0]) : 'Not specified'}</p>
      </div>
    `).join('');
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden;">
          ...
          <div style="padding: 20px; background: #ffffff;">
            ...
            <p><strong>Visa Interview Date:</strong> ${formData.additional.visaInterviewDate ? (formData.additional.visaInterviewDate.includes('T') ? new Date(formData.additional.visaInterviewDate.split('T')[0] + 'T12:00:00').toISOString().split('T')[0] : new Date(formData.additional.visaInterviewDate + 'T00:00:00').toISOString().split('T')[0]) : 'Not specified'}</p>
            <p><strong>Delivery Date:</strong> ${formData.additional.deliveryDate ? (formData.additional.deliveryDate.includes('T') ? new Date(formData.additional.deliveryDate.split('T')[0] + 'T12:00:00').toISOString().split('T')[0] : new Date(formData.additional.deliveryDate + 'T00:00:00').toISOString().split('T')[0]) : 'Not specified'}</p>
    ...
    const flightDates = formData.flight.legs.map(leg => 
      leg.date ? (leg.date.includes('T') ? new Date(leg.date.split('T')[0] + 'T12:00:00').toISOString().split('T')[0] : new Date(leg.date + 'T00:00:00').toISOString().split('T')[0]) : 'Not specified'
    ).join(', ');
