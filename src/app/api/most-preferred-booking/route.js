  ...
  try {
    ...
    const flightLegsHTML = formData.flight.legs.map((leg, index) => `
      <div style="margin-bottom: 15px; padding: 10px; border: 1px solid #e2e8f0; border-radius: 5px;">
        ...
        <p><strong>Date:</strong> ${leg.date ? (leg.date.includes('T') ? new Date(leg.date).toLocaleDateString('en-CA', { timeZone: 'Asia/Kolkata' }) : leg.date) : 'Not specified'}</p>
      </div>
    `).join('');
    ...
    const sheetData = {
      formName: formData.formName || 'Most Preferred Booking',
